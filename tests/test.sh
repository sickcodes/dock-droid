...
#
# Title:            dock-droid (Android on Docker)
...
REPO="${REPO:=https://github.com/sickcodes/dock-droid.git}"
...
clone_repo () {
    git clone --branch="${1}" "${2}" dock-droid
}
...
dock-droid:latest () {
    docker build ${NO_CACHE} \
        ...
        -f ./Dockerfile \
        -t dock-droid:latest .
    docker tag dock-droid:latest sickcodes/dock-droid:latest
}


# dock-droid:placeholder () {
#     docker build ${NO_CACHE} \
#         --build-arg RANKMIRRORS=true \
#         --build-arg MIRROR_COUNTRY="${MIRROR_COUNTRY}" \
#         -f ./Dockerfile.placeholder \
#         -t dock-droid:placeholder .
#     docker tag dock-droid:placeholder sickcodes/dock-droid:placeholder
# }
...
if [[ "${DOCKER_USERNAME}" ]] && [[ "${DOCKER_PASSWORD}" ]]; then
    docker login --username "${DOCKER_USERNAME}" --password "${DOCKER_PASSWORD}" \
        && docker push sickcodes/dock-droid:latest \
        && touch PUSHED
fi
